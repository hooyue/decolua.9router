# v0.5.81.1 (2026-09-19)

## Features
- **Providers**: merge the private v0.5.75.1 line onto upstream v0.5.81 — add the Volcengine Ark Agent Plan provider (`volcengine-agent`, alias `aap`) with OpenAI Chat Completions plus lossless Responses passthrough on `ark.cn-beijing.volces.com/api/plan/v3`
...
- **Dashboard**: Quota Tracker tables paginate at 4 rows per page and exhausted quotas (remaining 0, non-unlimited) sink to the back in every sort mode; card and filter titles use upstream v0.5.81's native `providerLabel`
